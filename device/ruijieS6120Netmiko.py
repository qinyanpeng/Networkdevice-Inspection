...
class ruijieS6120:
    ...
    def power(self):
        ...
        for i in range(0,3):
            try:
                ...
                state=False
        #print(output)
        match = re.findall(r'RG-PA150I-F\s+(\w+)',output)
        if len(match)==0:
            state=False
        else:
            for v in match:
    ...
    def fan(self):
        ...
        for i in range(0,3):
            try:
                ...
                state=False
        #print(output)
        match = re.findall(r'I-F\s+(\w+)',output)
        if len(match)==0:
            state=False
        else:
            for v in match:
    ...
    def version(self,threshold="RGOS12.6(2)B1302"):
        state=True
        version="none"
...
if __name__ == "__main__":
    ...
    for ip in ips:
        print(ip)
        device=ruijieS6120(ip,user,passwd)
        if device.is_alive:
            #cpu=device.cpu()
            #print(cpu)
            #if not cpu['state']:
