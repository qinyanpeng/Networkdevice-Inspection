...
class huaweiCE16804:
    ...
    def temperature(self):
        state=True
        command="display device temperature"
        output=''
        for i in range(0,2):
            output=self.command(command)
            print(output)
            match = re.findall(r'\s\s\s(\d+)\n',output)
            if len(match)!=0:
                state=True
                break
            else:
    ...
    def version(self,threshold="V200R022C00SPC500"):
        ...
        for i in range(0,2):
            ...
            match=re.search(r'[(]CE16800\s+(.*)[)]',output)
            if match:
                state=True
...
if __name__ == "__main__":
    ...
    device=huaweiCE16804(Ip,user,passwd)
